  ...
  <body class="news is-employer is-login">
    <div class="wrap">
      ...
      <main>
          ...
          <div class="inner">
            ...
            <section v-if="isFromFacility && ann.user_id" class="l-sec">
              <router-link
                ...
                :to="{
                  name: 'FacilityJobs',
                  params: { userId: ann.user_id },
                  query: { facility_name: facilityText },
                }"
              >
            ...
            <section v-if="isFeature && relatedJobs.length > 0" class="l-sec">
              ...
              <article
                ...
                :key="it.id"
                class="card clickable related-job-card"
              >
                <a
                  href="#"
                  class="stretched-link"
                  aria-label="求人詳細へ"
                  @click.prevent.stop="onOpenJob(it)"
                ></a>
        ...
        <!-- ✅ 未ログイン時の登録促し -->
        <div v-if="showAuthToast" class="apm-overlay" @click.self="showAuthToast = false">
          <div class="apm-modal" role="dialog" aria-modal="true" aria-labelledby="apm-title">
            <h3 id="apm-title" class="apm-title">マイページ登録しませんか？</h3>

            <button class="apm-cta" @click="router.push({ name: 'RegisterChoice' })">登録</button>
...
function buildDescriptionFromAnnouncement(a) {
  ...
  const body = a?.body ?? a?.content ?? a?.description ?? a?.text ?? "";
...
.facility-jobs-link:hover {
  filter: brightness(1.02);
}
.facility-jobs-link .arr {
  font-size: 18px;
  line-height: 1;
}
...
.btn-edit:focus-visible {
  outline: 3px solid rgba(94, 194, 255, 0.45);
  outline-offset: 2px;
}
...
.card :where(button, .actbar, .ico-btn, a:not(.stretched-link), input, select) {
  position: relative;
  z-index: 2;
}
