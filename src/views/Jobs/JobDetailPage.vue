          ...
          <div class="job-detail">
            <div class="inner">
              ...
              <div class="job-item__data">
                <span class="job-item__data--begins"
                  >募集開始：{{ formatDate(item.published_date) }}</span
                >
              ...
              <div class="status__items" v-if="!isEmployer">
                <div class="status__clip">
                  ...
                  <button
                    ...
                    :class="{ active: clipped, muted: !clipped }"
                    @click="toggleClip"
                  >
                ...
                <div class="status__good">
                  <button
                    class="js-modal-trigger"
                    :class="{ active: gooded, muted: !gooded }"
                    @click="toggleGood"
                  >
                ...
                <div class="status__talk">
                  <button
                    class="js-modal-trigger"
                    :class="{ active: talked, muted: !talked }"
                    @click="openTalk"
                  >
              ...
              <div class="job-item__summary flex">
                <div class="job-item__txtwrap">
                  ...
                  <div class="job-item__jobtype-row">
              ...
              <section class="job-item__detail">
                ...
                <dl class="table">
                  ...
                  <dt>募集職種</dt>
                  <dd>{{ jobPositionDisplay }}</dd>
                  <dt>学歴</dt>
                  <dd>{{ item.education_level_name || "-" }}</dd>
                  <dt>基本給</dt>
                  <dd>
                    {{
                      salaryLabel(
                        item.base_salary_type,
                        item.base_salary_min,
                        item.base_salary_max,
                      )
                    }}円
                  </dd>
                  <dt>処遇改善手当</dt>
                  ...
                  <dt>昇給</dt>
                  <dd>{{ item.raise_text || "-" }}</dd>
                  <dt>賞与</dt>
                  <dd>{{ item.bonus_text || "-" }}</dd>
                  <dt>休日</dt>
                  <dd>{{ item.holidays_text || "-" }}</dd>
                  <dt>保険加入</dt>
                  <dd>{{ item.insurance_text || "-" }}</dd>
                  <dt>募集人数</dt>
                  <dd>{{ item.headcount ?? "-" }}</dd>
                  <dt>シフト</dt>
                  <dd>{{ item.shift_text || "-" }}</dd>
                  <dt>勤務形態</dt>
                  <dd>{{ item.employment_type_name || "-" }}</dd>
                </dl>
              </section>
                ...
                <dl class="table">
                  ...
                  <dd>
                    {{
                      ...
                      (item.contact_last_name || "") + (item.contact_first_name || "") ||
                      "-"
                    }}
                  </dd>
                </dl>
...
const applyJobSeo = (job) => {
  ...
  const rawDesc = job?.pr_message || job?.description || job?.work_detail || job?.catch_copy || "";
...
const imageFrom = (jobId, p) => {
  ...
  if (/^\/upload\/jobs\//i.test(s))
    return `${ORIGIN}${s.replace(/^\/upload\/jobs\//i, "/uploads/images/jobs/")}`;
  if (/^\/uploads\/jobs\//i.test(s))
    return `${ORIGIN}${s.replace(/^\/uploads\/jobs\//i, "/uploads/images/jobs/")}`;
...
const fullAddress = computed(() => {
  ...
  return [d.prefecture_name, d.city, d.address_line1, d.address_line2].filter(Boolean).join("");
});
...
const fetchDetail = async () => {
  ...
  const normalized = {
    ...
    treatment_allowance:
      ...
      "",
    // 昇給
    raise_text: raw.raise_text ?? raw.raise ?? raw.salary_raise ?? "",
    // 賞与
    bonus_text: raw.bonus_text ?? raw.bonus ?? "",
    // 休日
    holidays_text: raw.holidays_text ?? raw.holiday_text ?? raw.holidays ?? "",
    // 保険加入
    insurance_text: raw.insurance_text ?? raw.insurance ?? "",
    // 募集人数
    headcount:
      raw.headcount ?? raw.recruitment_number ?? raw.recruit_number ?? raw.recruitment ?? null,
    // シフト名（マスタ結合して返している／いない両対応）
    shift_text: raw.shift_text ?? raw.shift_name ?? raw.shift ?? "",
    // 勤務時間名（マスタ結合して返している／いない両対応）
    working_time_text: raw.working_time_text ?? raw.work_time_name ?? raw.work_time ?? "",
  };
