      ...
      <div class="content">
        <main>
          <section class="l-sec mypage-detail">
            <div class="inner">
              <div class="page-title maru">{{ title }}（全 {{ total }} 件）</div>
              <!-- <div class="actions">
                  ...
                  <div class="btn-job-edit-items flex">
                    ...
                    <template v-if="props.status !== 'draft'">
                      ...
                      <template v-else>
                        ...
                        <button
                          class="btn-job-edit-item"
                          @click.stop="askClose(it)"
                          :disabled="!!it.is_closed"
                        >
                          終了
                        </button>
                      </template>
...
const props = defineProps({
  ...
  status: {
    type: String,
    default: "open",
    validator: (v) => ["open", "paused", "closed", "draft"].includes(v),
  },
});
...
function askDelete(item) {
  modal.value = { open: true, type: "delete", item };
}
...
function askPause(item) {
  modal.value = { open: true, type: "pause", item };
}
function askClose(item) {
  modal.value = { open: true, type: "close", item };
}
function askReopen(item) {
  modal.value = { open: true, type: "reopen", item };
}
...
const confirmMessage = computed(() => {
  ...
  if (m.type === "pause")
    return `この求人の募集を${m.item.is_paused ? "再開" : "休止"}します。よろしいですか？`;
  if (m.type === "close") return "この求人の終了をします。よろしいですか？";
  if (m.type === "reopen") return "この求人の募集を再開します。よろしいですか？";
  if (m.type === "delete")
    return "この求人を完全に削除します。関連データも消去され元に戻せません。実行しますか？";
  return "";
});
...
async function doPause() {
  ...
  try {
    ...
    notice.value = {
      open: true,
      message: toPause ? "募集を休止しました。" : "募集を再開しました。",
    };
  } catch (e) {
...
:deep(.job-card__footer > .icon-btn) {
