...
const initialDraftApplied = ref(false);

/**
 * 求人詳細ページから渡された応募文を入力欄にセットする
 */
function applyInitialDraft() {
  const text = String(props.initialDraft || "").trim();

  if (!text) return;

  const current = String(draft.value || "").trim();

  // すでに同じ文面が入っている場合は何もしない
  if (current.includes(text)) {
    initialDraftApplied.value = true;
    return;
  }

  // 一度適用済みで、かつ現在の入力欄に別の文字がある場合は重複防止
  if (initialDraftApplied.value && current) {
    return;
  }
...
const DRAFT_KEY = computed(() => `talk:${props.role}:${props.userId}:${props.threadId}`);
...
function isMyMessage(m) {
  return m.sender_type === props.role && m.sender_id === props.userId;
}
function rowClass(m) {
  return isMyMessage(m) ? "me" : "you";
}
...
async function load() {
  ...
  try {
    ...
    const res = await http.get(`/talks/${props.threadId}/messages`, {
...
function goBack() {
  if (props.backTo) {
    try {
      ...
      window.location.assign(props.backTo);
    }
    return;
  }
  ...
  try {
    ...
    window.location.assign(isEmployer ? "/biz/talks" : "/jobs/talks");
  }
}
...
onMounted(async () => {
  // load() の成功・失敗に関係なく、まず応募文を反映
  applyInitialDraft();

  try {
    await load();
  } catch (e) {
    console.error("[TalkPanel] load failed", e);
  }
